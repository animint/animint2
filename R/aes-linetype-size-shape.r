...
#'   a_geom_line(aes(linetype = variable))
#'
#' # Size examples
...
#' s + a_geom_point(aes(shape = z), size = 4) +
#'   a_scale_shape_identity()
#' # While all symbols have a foreground colour, symbols 19-25 also take a
#' # background colour (fill)
#' s + a_geom_point(aes(shape = z), size = 4, colour = "Red") +
#'   a_scale_shape_identity()
#' s + a_geom_point(aes(shape = z), size = 4, colour = "Red", fill = "Black") +
#'   a_scale_shape_identity()
NULL
