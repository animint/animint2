...
#' # qplot(x, y, data = data)
#' # a_plot(data, aes(x, y)) + a_geom_point()
#'
#' # Using Aesthetics
#'
...
#' # a_plot(data, aes(x, y, shape = shape, colour = colour)) + a_geom_point()
#' #
...
#' # a_plot(data, aes(x, y)) + a_geom_point(method = "lm") + a_geom_smooth(method = "lm")
#'
#' # Scales and axes
#'
...
#' # a_plot(data, aes(x, y)) + a_geom_point() + a_scale_x_log10() + a_scale_y_log10()
...
#' # a_plot(data, aes(x, y)) + a_geom_point() + labs(title = "title") + a_theme(aspect.ratio = 1)
NULL
