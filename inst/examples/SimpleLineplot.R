...
#' Simple line plot
p1 <- a_plot() + a_geom_line(data=data, a_aes(x=x, y=y, group=group)) + 
  ggtitle("a_geom_line")
p1
...
p2 <- a_plot() + a_geom_line(data=data, a_aes(x=x, y=y, colour=group, group=group)) +
  ggtitle("a_geom_line + a_scale_colour_discrete")
p2
# gg2animint(list(p1=p1, p2=p2))

#' Simple line plot with colors and linetype
p3 <- a_plot() + a_geom_line(data=data, a_aes(x=x, y=y, colour=group, group=group, linetype=lt)) +
  ggtitle("a_geom_line + a_scale_linetype_manual")
p3
# gg2animint(list(p1=p1, p2=p2, p3=p3))

#' Use automatic linetypes from ggplot with coerced factors
p4 <- a_plot() + a_geom_line(data=data, a_aes(x=x, y=y, colour=group, group=group, linetype=group)) +
  ggtitle("a_geom_line + a_scale_linetype automatic")
p4
...
p5 <- a_plot() + a_geom_line(data=data, a_aes(x=x, y=y, colour=group, group=group, linetype=lt)) + 
  a_scale_linetype_identity("group", a_guide="legend", a_labels = c("1", "2", "3", "4", "5")) + 
...
p6 <- a_plot()+a_geom_segment(data=lt1, a_aes(x=x, xend=xend, y=y, yend=yend, linetype=lt)) + 
  a_scale_linetype_identity() + a_geom_text(data=lt1, a_aes(x=lx, y=y, a_label=lt), hjust=0) + 
...
p7 <- a_plot() + a_geom_segment(data=lt2, a_aes(x=x, xend=xend, y=y, yend=yend, linetype=lt)) + 
  a_scale_linetype_identity() + a_geom_text(data=lt2, a_aes(x=lx, y=y, a_label=lt), hjust=0) +
...
qplot(data=data2, x=x, y=y, group=group, a_geom="line", alpha=I(.2))

#' Check whether a_scale_alpha works.
p8 <- a_plot() + a_geom_line(data=data2, a_aes(x=x, y=y, group=group), alpha=.1) +
  ggtitle("Constant alpha")
p8
# gg2animint(list(p1=p1, p2=p2, p3=p3, p4=p4, p5=p5, p6=p6, p7=p7, p8=p8))

p9 <- a_plot() + a_geom_line(data=subset(data2, as.numeric(group) < 50), 
                           a_aes(x=x, y=y, group=group, linetype=lty), alpha=.2) +
  a_scale_linetype_identity() + 
  ggtitle("Constant alpha, I(linetype)")
p9
# gg2animint(list(p1=p1, p2=p2, p3=p3, p4=p4, p5=p5, p6=p6, p7=p7, p8=p8, p9=p9))

p10 <- a_plot() + a_geom_line(data=subset(data2, as.numeric(group) < 50), 
                            a_aes(x=x, y=y, group=group, linetype=below0, alpha=maxy)) + 
  a_scale_alpha_continuous(range=c(.1, .5)) + 
  ggtitle("Continuous alpha")
p10
# gg2animint(list(p1=p1, p2=p2, p3=p3, p4=p4, p5=p5, p6=p6, p7=p7, p8=p8, p9=p9, p10=p10))

#' Size Scaling
p11 <- a_plot() + a_geom_line(data=subset(data2, as.numeric(group)%%50 ==1), 
                            a_aes(x=x, y=y, group=group, size=(floor(miny)+3)/3)) + 
  a_scale_size_continuous("Line Size", range=c(1,3)) +
  ggtitle("Continuous size")
p11
# gg2animint(list(p1=p1, p2=p2, p3=p3, p4=p4, p5=p5, p6=p6, p7=p7, p8=p8, p9=p9, p10=p10, p11=p11))

p12 <- a_plot() + a_geom_line(data=data2, a_aes(x=x, y=y, group=group, alpha=miny, colour=maxy)) + 
  a_scale_alpha_continuous(range=c(.1, .3)) + 
