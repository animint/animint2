...
p2 <- a_plot() + a_geom_line(data=data, aes(x=x, y=y, colour=group, group=group)) +
  ggtitle("a_geom_line + a_scale_colour_discrete")
p2
# gg2animint(list(p1=p1, p2=p2))

#' Simple line plot with colors and linetype
p3 <- a_plot() + a_geom_line(data=data, aes(x=x, y=y, colour=group, group=group, linetype=lt)) +
...
p4 <- a_plot() + a_geom_line(data=data, aes(x=x, y=y, colour=group, group=group, linetype=group)) +
...
p6 <- a_plot()+a_geom_segment(data=lt1, aes(x=x, xend=xend, y=y, yend=yend, linetype=lt)) + 
  a_scale_linetype_identity() + a_geom_text(data=lt1, aes(x=lx, y=y, a_label=lt), hjust=0) + 
...
p7 <- a_plot() + a_geom_segment(data=lt2, aes(x=x, xend=xend, y=y, yend=yend, linetype=lt)) + 
  a_scale_linetype_identity() + a_geom_text(data=lt2, aes(x=lx, y=y, a_label=lt), hjust=0) +
...
p8 <- a_plot() + a_geom_line(data=data2, aes(x=x, y=y, group=group), alpha=.1) +
...
p12 <- a_plot() + a_geom_line(data=data2, aes(x=x, y=y, group=group, alpha=miny, colour=maxy)) + 
