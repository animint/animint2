...
[Animint](https://github.com/tdhock/animint) is an R package for creating web-based interactive graphics and animations using a_plot2’s grammar of graphics approach. Now you can embed animint plots inside [shiny apps](http://shiny.rstudio.com/). In the shiny app below, you can change the x/y/color variables and both plots will update.
...
Now, we construct a time-series and scatterplot using a_plot2's grammar with the addition of animint's `clickSelects` and `showSelected` aesthetics to create an interactive link between the plots.
...
getPlotList <- reactive({
  # think of animint objects as a list of a_plots and specially named options
