...
var animint = function (to_select, json_file) {
  ...
  var nest_by_group = d3.nest().key(function (d) {
    return d.group;
  });
  ...
  var add_plot = function (p_name, p_info) {
    ...
    var xtitlepadding =
      5 + measureText(p_info['xtitle'], default_axis_px).height;
    var ytitlepadding =
      5 + measureText(p_info['ytitle'], default_axis_px).height;
    ...
    svg
      ...
      .attr(
        'transform',
        'translate(' + plotdim.title.x + ',' + plotdim.title.y + ')'
      )
    ...
    if (p_info.hasOwnProperty('ylabs') && p_info.ylabs.length) {
      axispaddingy += Math.max.apply(
        null,
        p_info.ylabs.map(function (entry) {
    ...
    if (p_info.hasOwnProperty('xlabs') && p_info.xlabs.length) {
      ...
      axispaddingx += Math.max.apply(
        null,
        p_info.xlabs.map(function (entry) {
    ...
    var graph_width =
      p_info.options.width -
      ncols * (margin.left + margin.right) -
      strip_width -
      n_yaxes * axispaddingy -
      ytitlepadding;
    var graph_height =
      p_info.options.height -
      nrows * (margin.top + margin.bottom) -
      strip_height -
      titlepadding -
      n_xaxes * axispaddingx -
      xtitlepadding;
    ...
    var topStrip = svg.append('g').attr('class', 'topStrip');
    var rightStrip = svg.append('g').attr('class', 'rightStrip');
    ...
    for (var layout_i = 0; layout_i < npanels; layout_i++) {
      ...
      // axes.
      scales[panel_i] = {};
      scales[panel_i].x = d3.scale
        .linear()
        .domain(axis.xrange)
        .range([plotdim.xstart, plotdim.xend]);
      scales[panel_i].y = d3.scale
        .linear()
        .domain(axis.yrange)
      ...
      if (draw_y) {
        ...
        yaxis_g.selectAll('.tick text').style('font-size', p_info.ysize);
      }
      ...
      if (Object.keys(p_info.panel_background).length > 1) {
        background
          ...
          .style('stroke-dasharray', function () {
            return linetypesize2dasharray(
              p_info.panel_background.linetype,
      ...
      ['grid_minor', 'grid_major'].forEach(function (grid_class) {
        ...
        if (grid_background.hasOwnProperty('size')) {
          var grid = background.append('g').attr('class', grid_class);
          ['x', 'y'].forEach(function (scale_var) {
            ...
            grid
              ...
              .style(
                'stroke-dasharray',
                linetypesize2dasharray(
                  grid_background.linetype,
                  grid_background.size
                )
              );
          });
        }
    ...
    if (p_info['xtitle']) {
      svg
        ...
        .attr(
          'transform',
          'translate(' + (xtitle_left + xtitle_right) / 2 + ',' + xtitle_y + ')'
        );
    }
  ...
  var add_selector = function (s_name, s_info) {
    ...
    s_info.legend_tds = element.selectAll(
      'tr.' + legend_class_name(s_name) + ' td.legend_entry_label'
    );
  ...
  var draw_geom = function (g_info, chunk, selector_name, PANEL) {
    ...
    var has_clickSelects_variable = g_info.aes.hasOwnProperty(
    ...
    var fill_comes_from = 'fill',
      fill_off_comes_from = 'fill_off';
    if (g_info.data_is_object) {
      ...
      if (g_info.geom == 'ribbon') {
        var lineThing = d3.svg
          .area()
          .x(toXY('x', 'x'))
          .y(toXY('y', 'ymax'))
          .y0(toXY('y', 'ymin'));
      } else {
        var lineThing = d3.svg.line().x(toXY('x', 'x')).y(toXY('y', 'y'));
      }
      ...
      if (g_info.geom == 'text') {
        ...
        g_info.style_list = ['opacity', 'fill'];
        eActions = function (e) {
          e.attr('x', toXY('x', 'x'))
            .attr('y', toXY('y', 'y'))
      ...
      if (g_info.geom == 'point') {
        ...
        if (
          !(g_info.params.hasOwnProperty('fill') || aes.hasOwnProperty('fill'))
        ) {
          fill_comes_from = 'colour';
        }
    ...
    g_info.select_style = select_style_default.filter((X) =>
      g_info.style_list.includes(X)
    );
    ...
    if (g_info.aes.hasOwnProperty('href')) {
      enter = enter.append('svg:a').append('svg:' + eAppend);
    } else {
      enter = enter.append(eAppend).attr('class', 'geom');
    }
    ...
    if (has_clickSelects || has_tooltip || has_clickSelects_variable) {
      ...
      elements.append('svg:title').text(get_fun(text_fun));
    }
    ...
    if (axis_domains != null) {
      axes.forEach(function (xyaxis) {
        ...
        Plots[p_name].layout.PANEL.forEach(function (panel_i, i) {
          ...
          var draw_axes =
            Plots[p_name].layout['AXIS_' + xyaxis.toUpperCase()][i];
          if (draw_axes) {
  ...
  function update_axes(p_name, axes, panel_i, tick_vals) {
    ...
    var xyaxis_g = element
      .select('#plot_' + p_name)
      .select('.' + axes + 'axis_' + panel_i)
      .transition()
      .duration(1000)
      .call(xyaxis);
  }
  ...
  var add_legend = function (p_name, p_info) {
    ...
    for (var i = 0; i < legendkeys.length; i++) {
      ...
      var legend_table = tdRight.append('table').attr('class', 'legend');
      ...
      if (l_info.geoms.indexOf('polygon') > -1) {
        ...
        legend_svgs
          ...
          .style('stroke-width', function (d) {
            return d['polygonsize'] || 1;
          })
          .style('stroke-dasharray', function (d) {
            return linetypesize2dasharray(d['polygonlinetype'], d['size'] || 2);
          })
          .style('stroke', function (d) {
            return d['polygoncolour'] || '#000000';
          })
          .style('fill', function (d) {
            return d['polygonfill'] || '#FFFFFF';
          })
          .style('opacity', function (d) {
            return d['polygonalpha'] || 1;
          });
      }
      ...
      if (l_info.geoms.indexOf('path') > -1) {
        ...
        legend_svgs
          ...
          .style('stroke', function (d) {
            return d['pathcolour'] || '#000000';
          })
          .style('opacity', function (d) {
            return d['pathalpha'] || 1;
          });
      }
      if (l_info.geoms.indexOf('point') > -1) {
        ...
        legend_svgs
          ...
          .attr('cy', 7)
          .attr('r', function (d) {
            return pointscale(d['pointsize']) || 4;
          })
          .style('stroke', function (d) {
            return d['pointcolour'] || '#000000';
          })
          .style('fill', function (d) {
            return d['pointfill'] || d['pointcolour'] || '#000000';
          })
          .style('opacity', function (d) {
            return d['pointalpha'] || 1;
          });
      }
      legend_rows
        .append('td')
  ...
  d3.json(json_file, function (error, response) {
    ...
    var loading = widget_td.append('table').style('display', 'none');
    ...
    var show_hide_animation_controls = widget_td
      .append('button')
      .text(show_message)
    ...
    var time_table = widget_td.append('table').style('display', 'none');
    ...
    var duration_rows = time_table
      .selectAll('tr.duration')
      .data(selector_array)
      .enter()
      .append('tr');
    duration_rows.append('td').text(function (s_name) {
      return s_name;
    });
    ...
    var selector_table = widget_td
      .append('table')
      .style('display', 'none')
      .attr('class', 'table_selector_widgets');
    var selector_first_tr = selector_table.append('tr');
    selector_first_tr.append('th').text('Variable');
    selector_first_tr.append('th').text('Selected value(s)');
    ...
    for (s_name in Selectors) {
      ...
      if (isArray(s_info.levels)) {
        ...
        var has_no_legend = !Selectors[s_name].hasOwnProperty('legend');
        ...
        var selector_widget_row = selector_table
          .append('tr')
          .attr('class', function () {
            return s_name_id + '_selector_widget';
          });
        ...
        var selector_widget_select = selector_widget_row
          ...
          .append('select')
          .attr('class', function () {
            return s_name_id + '_input';
          })
          .attr('placeholder', function () {
            return 'Toggle ' + s_name;
          });
        ...
        if (s_info.type == 'single') {
          ...
          var $temp = $(selectize_selector).selectize({
            create: false,
            valueField: 'id',
            labelField: 'text',
            searchField: ['text'],
    ...
    var check_func = function () {
      ...
      return status_array.every(function (elem) {
    ...
    if (window.location.hash) {
      ...
      frag_array = frag_array.filter(function (x) {
        return x != '';
      });
