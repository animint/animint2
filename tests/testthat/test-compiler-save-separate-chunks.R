...
### test case 2
USdots <- data.table(FluView$USpolygons)[, list(mean.lat = mean(lat), mean.long = mean(long)), by = region]
# add state flu to points.
flu.points <- do.call(rbind, lapply(unique(state_flu$WEEKEND), function(we) {
