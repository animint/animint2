...
test_that("no minor breaks when only one break", {
  sc1 <- a_scale_x_discrete(limits = "a")
  sc2 <- a_scale_x_continuous(limits = 1)
...
test_that("scale breaks with numeric log transformation", {
  sc <- a_scale_x_continuous(limits = c(1, 1e5), trans = log10_trans())
...
test_that("suppressing breaks, minor_breask, and labels", {
  expect_equal(a_scale_x_continuous(breaks = NULL, limits = c(1, 3))$get_breaks(), NULL)
...
test_that("a_scale_breaks with explicit NA options (deprecated)", {
  # NA is defunct, should throw error

  # X
  sxc <- a_scale_x_continuous(breaks = NA)
  sxc$train(1:3)
  expect_error(sxc$get_breaks())
  expect_error(sxc$get_breaks_minor())

  # Y
  syc <- a_scale_y_continuous(breaks = NA)
  syc$train(1:3)
  expect_error(syc$get_breaks())
  expect_error(syc$get_breaks_minor())

  # Alpha
  sac <- a_scale_alpha_continuous(breaks = NA)
  sac$train(1:3)
  expect_error(sac$get_breaks())

  # Size
  ssc <- a_scale_size_continuous(breaks = NA)
  ssc$train(1:3)
  expect_error(ssc$get_breaks())

  # Fill
  sfc <- a_scale_fill_continuous(breaks = NA)
  sfc$train(1:3)
  expect_error(sfc$get_breaks())

  # Colour
  scc <- a_scale_colour_continuous(breaks = NA)
  scc$train(1:3)
...
test_that("only finite or NA values for breaks for transformed scales (#871)", {
  sc <- a_scale_y_continuous(limits = c(0.01, 0.99), trans = "probit",
                           breaks = seq(0, 1, 0.2))
...
test_that("minor breaks are transformed by scales", {
  sc <- a_scale_y_continuous(limits = c(1, 100), trans = "log10",
