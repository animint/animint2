...
test_that("coord_fixed() + a_facet_wrap(nrow=1) with shrunk y-axis", {
  ratio2 <- 2
  wrapViz <-
    list(wrapPlot=p +
         a_facet_wrap(~am) + coord_fixed(ratio2))
...
test_that("coord_fixed() + a_facet_wrap(nrow=1) with shrunk x-axis", {
  ratio10 <- 10
  wrapViz <-
    list(wrapPlot=p +
           a_facet_wrap(~am) + coord_fixed(ratio10))
  info <- animint2HTML(wrapViz)
  x.axes <- getNodeSet(info$html, "//g[contains(@class, 'xaxis')]")
  y.axes <- getNodeSet(info$html, "//g[contains(@class, 'yaxis')]")
  xdiff1 <- getTickDiff(x.axes[[1]])
