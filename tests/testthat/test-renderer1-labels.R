...
ggpoint <- a_plot() + a_geom_point(aes(Petal.Width, Sepal.Width), data = iris)
...
test_that("a_scale_x_continuous(breaks)+xlab(name) converts", {
  viz <-
    list(scatter=a_plot() +
           a_geom_point(aes(Petal.Length, Sepal.Length), data = iris) +
...
series <- a_plot() + a_geom_line(aes(x = time, y = value, group = vars), data = stocks)
